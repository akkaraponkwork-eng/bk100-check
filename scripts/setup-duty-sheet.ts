import { GoogleSheetsClient } from '../src/lib/repository/sheets/client';
import DutyAssignmentDocType from '../src/doctypes/DutyAssignment.json';
import dotenv from 'dotenv';
dotenv.config({ path: '.env.local' });

async function setupSheet() {
  console.log('Setting up Google Sheet...');
  const { sheets, sheetId } = GoogleSheetsClient.getInstance();

  try {
    console.log(`Creating tab: ${DutyAssignmentDocType.sheetName}`);
    try {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: sheetId,
        requestBody: {
          requests: [
            {
              addSheet: {
                properties: {
                  title: DutyAssignmentDocType.sheetName,
                }
              }
            }
          ]
        }
      });
      console.log('✅ Tab created successfully.');
    } catch (e: any) {
      if (e.message.includes('already exists')) {
        console.log('ℹ️ Tab already exists, proceeding to setup headers.');
      } else {
        throw e;
      }
    }

    console.log('Setting up headers...');
    const headers = DutyAssignmentDocType.fields
      .filter((f: any) => f.sheetHeader)
      .map((f: any) => f.sheetHeader);

    await sheets.spreadsheets.values.update({
      spreadsheetId: sheetId,
      range: `${DutyAssignmentDocType.sheetName}!A1`,
      valueInputOption: 'USER_ENTERED',
      requestBody: {
        values: [headers]
      }
    });

    console.log(`✅ Headers configured: ${headers.join(', ')}`);
    console.log('🎉 Setup complete! You can now test the Duty Workflow.');
    
  } catch (error) {
    console.error('Setup failed:', error);
  }
}

setupSheet();
