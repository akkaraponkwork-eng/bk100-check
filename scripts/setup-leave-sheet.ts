...
async function setupSheet() {
  ...
  try {
    console.log(`Creating tab: ${LeaveRequestDocType.sheetName}`);
    try {
          ...
          requests: [
            {
              addSheet: {
                properties: {
                  title: LeaveRequestDocType.sheetName,
                }
              }
            }
          ]
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: sheetId,
      range: `${LeaveRequestDocType.sheetName}!A1`,
    ...
    console.log('🎉 Setup complete! You can now test the Leave Workflow.');
